...
namespace BoardIdentityServer.Controllers
{
    ...
    public class AuthenticateController : Controller
    {
        ...
        [HttpGet]
        [Route("Google")]
        public async Task<IActionResult> Google([Required]string returnUrl)
        {
            ...
            {
                RedirectUri = Url.Action(nameof(HandleGoogleCallback)).ToLower(),
                Items = {
                    { "returnUrl", returnUrl },
                    { "scheme", "Google" }
                }
        ...
        public async Task<IActionResult> HandleGoogleCallback()
        {
            ...
            if (user == null)
            {
                ...
                user = new User()
                {
                    ...
                    PictureUrl = externalUser.FindFirst(JwtClaimTypes.Picture).Value,
                    Locale = externalUser.FindFirst(JwtClaimTypes.Locale).Value,
                    Provider = provider
                };
            ...
            var identityServerUser = new IdentityServerUser(user.ExternalId)
            {
